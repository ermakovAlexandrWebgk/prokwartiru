...
<!-- Заголовок страницы -->
<?
  $page_title="<h1 id='page_title'><span>".$section["NAME"]."</span>";
...
<!-- Навигация -->
<div id="page_navigation" style="margin-top: -20px; padding-bottom: 20px;">
...
<!-- Заголовок -->
  <div class="item-title" style="height: 30px; overflow: hidden;"><span>Арт.</span> <a href="<?=$arElement['DETAIL_PAGE_URL']?>" title="<?=$arElement['NAME']?>"><b><?=$arElement["NAME"]?></b></a><?=$sticker?></div>
<!-- Конец Заголовок -->
...
<? 
       ...
       if($iblock_ID==10) $title_type=$title_type." мозаика";
...
<?
	if (is_array($arElement['DISPLAY_PROPERTIES']) && count($arElement['DISPLAY_PROPERTIES']) > 0){
		...
		if($fabrika['NAME']) $text=$text."Фабрика: <b>".$fabrika['NAME']."</b><br />";
		if($country['NAME']) $text=$text."Страна: <b>".$country['NAME']."</b><br />";
