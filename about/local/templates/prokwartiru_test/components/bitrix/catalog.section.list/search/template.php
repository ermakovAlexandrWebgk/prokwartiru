...
if(CModule::IncludeModule("iblock"))
{
   ...
   while($arItem = $items->GetNext())
   {
	...
	if($arItem['ID']==$fabrikaID) $fabriki = $fabriki."<br /><span style='position: absolute; left: -20px;'><b style='color:#322f2a;'>".$letter."</b></span><span class='orange'>".$arItem["NAME"]."</span>";
          ...
          $fabriki=$fabriki."<br /><input type='checkbox' name='fabrika' value='".$arItem['ID']."' id='fabrika_".$arItem['ID']."' onclick='javascript: if (FABRIKA_ID) document.getElementById(\"fabrika_\"+FABRIKA_ID).checked=false; if(this.value!=FABRIKA_ID) FABRIKA_ID=this.value; else FABRIKA_ID=\"\";' ";
          if($arItem['ID']==$FABRIKA_ID) $fabriki=$fabriki." checked";
          $fabriki=$fabriki.">".$arItem["NAME"]."</a>";

	//if($arItem['ID']==$fabrikaID){
        //  $fabriki = $fabriki."<br /><input type='checkbox' name='fabrika' value='".$arItem['ID']."' checked>".$arItem["NAME"];
        //}
	//else
        //{
        //  $fabriki = $fabriki."<br /><input type='checkbox' name='fabrika' value='fabrika".$arItem['ID']."' onclick='javascript: FABRIKA_ID=".$arItem['ID'].";'><a class='fabriki' href='".$arSite."index.php?FABRIKA_ID=".$arItem['ID'];
