...
{
// Выведем актуальную корзину для текущего постьльзователя
	global $arPageItems;
	$arBasketItems = array();
...
<?$section=GetIBlockSection($arParams["SECTION_ID"], 'catalog');?>
<!--GetIBlockSection($fabrika['IBLOCK_SECTION_ID']-->
...
?>
<?if($col==2) { $row++; $col=1; echo("<div style='clear:both; margin:0; padding:0 5px;'><!--hr--></div>"); } else $col++;?>
	<div class="catalog-item-card">
	  <!-- Заголовок -->
	  <div class="item-title"><span>Арт.</span> <a href="<?=$arElement['DETAIL_PAGE_URL']?>" target="_self" title="<?=$arElement['NAME']?>"><b><?=$arElement["NAME"]?></b></a><?=$sticker?></div>
	<?$title=$arElement['NAME'];?>
	  <!-- Конец Заголовок -->

	  <div class="item-info">
	    <!-- Текст описания товара -->
	    <div class="item-desc" style="margin:0;">
		...
		<?
		  foreach($arElement["PRICES"] as $code=>$arPrice){
			...
			$unit="";
			if($arElement['PROPERTIES']['UNIT']['VALUE']) $unit=$arElement['PROPERTIES']['UNIT']['VALUE']; else $unit="шт.";
			$price="<span class='float-card-price'>".$price."</span> / ".$unit;
		    }
		  }
		?>
		<!-- Конец Цена Товара -->
...
$arElement['ADD_URL']="/catalog/lepnina/?action=ADD2BASKET&id=".$arElement['ID']."&SECTION_ID=".$arElement['IBLOCK_SECTION_ID'];
		    if(!inBasket($arElement['ID'])) $button_buy=$button_buy."<div class='buttons_' style='margin: 0px;'><a href='javascript:;' class='catalog-item-buy' rel='nofollow'  onclick='javascript: Add2Basket(&quot;".$arElement['ADD_URL']."&quot;, &quot;".$arElement['ID']."&quot;);' id='catalog_add2cart1_link_".$arElement['ID']."'>".GetMessage('CATALOG_ADD')."</a></div>";	
		...
		<?$title_text="<table cellspacing='0' cellpadding='0' border='0' class='float-card'><tr><td class='left' valign='top' style='min-width: 250px;'>".$button_buy."<p>Артикул: <b class='big'>".$title."</b></p><p>Цена: ".$price."</p><p>Наличие: <b class='orange'>".$quantity."</b></p></td>";
		...
		<!-- Картинка -->
		<div class="item-image">
		  <a class="fancybox" data-fancybox-group="gallery" href="#inline_<?=$arElement['ID']?>"><img src="<?if($arElement['DETAIL_PICTURE']['SRC']):?><?=$arElement['DETAIL_PICTURE']['SRC']?><?else:?><?=SITE_TEMPLATE_PATH?>/images/default150.gif><?endif?>" alt="Арт. <?=$arElement["NAME"]?>" title="Открыть картинку" id="catalog_list_image_<?=$arElement['ID']?>"/></a>
...
<script language="JavaScript">document.write('<div id="inline_<?=$arElement['ID']?>" style="width:'+winW+'px; height:'+winH+'px; display:'+display+'; text-align: center;">');</script>
		    ...
		    <table style="margin: auto;"><tr><td valign="top" style="text-align:center; min-width: 250px;">
...
<!--?if($n==1):?-></td></tr><tr><td><hr><-?$n=0;?-><-?else:?-><-?$n++;?-><-?endif?-->
<?endforeach;?> 
