...
<?
  if(($arSite=="/catalog/fabriki/")&&(CModule::IncludeModule("iblock"))){
	...
	if($SECTION_ID=="51") { $window_title="Обои "; $keywords="Обои "; $IBLOCK_ID=5; } /* обои */
	if($SECTION_ID=="78") { $window_title="Плитка "; $keywords="Плитка "; $IBLOCK_ID=4; } /* плитка */
	if($SECTION_ID=="140") { $window_title="Мозаика "; $keywords="Мозаика "; $IBLOCK_ID=10; } /* мозаика */
	if($SECTION_ID=="759") { $window_title="Светильники "; $keywords="Светильники "; $IBLOCK_ID=17; } /* свет */
	if($SECTION_ID=="1266") { $window_title="Лепнина "; $keywords="Лепнина ";$IBLOCK_ID=20; } /* лепнина */
...
<!-- Добавление фотообоев в детские SECTION_ID 679 и 826 -->
<?if($section==58):?>
<?
  $id=array(679, 826);
  for( $i=0; $i<count($id); $i++)
  {
	...
	while($ar_catalogPhoto=$catalogPhoto->GetNext()) 
  	...
  	?>
	<div class="catalog-item">
	  <div class="catalog-item-image"><a href="<?=$ar_catalogPhoto['SECTION_PAGE_URL']?>"><?if($ar_catalogPhoto["PICTURE"]):?><? echo(ShowImage($ar_catalogPhoto["PICTURE"], 150, 150, "border='0' title='Открыть каталог'", "", true));?><?else:?><img src="<?=SITE_TEMPLATE_PATH?>/images/default150.gif" width="150px" height="150px" title="Открыть каталог"><?endif?></a></div>
  ...
  <?}?>
	<?}?>
<?endif?>
<!-- Конец Добавление фотообоев в детские -->
...
<?$APPLICATION->SetPageProperty("keywords", "каталог ".$keywords.", ".$keywords.", ".$keywords." цена, ".$keywords." купить");?>
