...
<div class="catalog-detail" style="height:auto;">
...
<?
		...
		if($arResult['PROPERTIES']['PROPERTY']['VALUE']) $text=$text."<p>Основные характеристики: <b>".$arResult['PROPERTIES']['PROPERTY']['VALUE']."</b></p>";
		...
		if($arResult['PROPERTIES']['DESIGN']['VALUE']) $text=$text."<p>Фактура: <b>".$arResult['PROPERTIES']['DESIGN']['VALUE']."</b></p>";
		if($arResult['PROPERTIES']['COLOR']['VALUE']) $text=$text."<p>Цвет: <b>".$arResult['PROPERTIES']['COLOR']['VALUE']."</b></p>";
		if($arResult['PROPERTIES']['CLASS']['VALUE']) $text=$text."<p>Класс: <b>".$arResult['PROPERTIES']['CLASS']['VALUE']."</b></p>";
		if($arResult['PROPERTIES']['COVER']['VALUE']) $text=$text."<p>Покрытие: <b>".$arResult['PROPERTIES']['COVER']['VALUE']."</b></p>";
		if($arResult['PROPERTIES']['LOCK']['VALUE']) $text=$text."<p>Замковая система: <b>".$arResult['PROPERTIES']['LOCK']['VALUE']."</b></p>";
		if($arResult['PROPERTIES']['SURFACE']['VALUE']) $text=$text."<p>Поверхность: <b>".$arResult['PROPERTIES']['SURFACE']['VALUE']."</b></p>";
		if($arResult['PROPERTIES']['PERIOD']['VALUE']) $text=$text."<p>Срок службы: <b>".$arResult['PROPERTIES']['PERIOD']['VALUE']."</b></p>";
		if($arResult['PROPERTIES']['LAMEL']['VALUE']) $text=$text."<p>Количество полос: <b>".$arResult['PROPERTIES']['LAMEL']['VALUE']."</b></p>";
		if($arResult['PROPERTIES']['FASKA']['VALUE']) $text=$text."<p>Фаска: <b>".$arResult['PROPERTIES']['FASKA']['VALUE']."</b></p>";


		if($arResult['PROPERTIES']['UPAK_KBM']['VALUE']||$arResult['PROPERTIES']['UPAK_KBM']['VALUE']||$arResult['PROPERTIES']['UPAK_PCS']['VALUE']||$arResult['PROPERTIES']['UPAK_KG']['VALUE']) $text=$text."<p>Упаковка: ";
		if($arResult['PROPERTIES']['UPAK_KBM']['VALUE']) $text=$text."<b>".$arResult['PROPERTIES']['UPAK_KBM']['VALUE']." кв.м</b>";
		if($arResult['PROPERTIES']['UPAK_PCS']['VALUE']) { if($arResult['PROPERTIES']['UPAK_KBM']['VALUE']) $text=$text." / "; $text=$text."<b>".$arResult['PROPERTIES']['UPAK_PCS']['VALUE']." шт.</b>"; }
		if($arResult['PROPERTIES']['UPAK_KG']['VALUE']) { if($arResult['PROPERTIES']['UPAK_PCS']['VALUE']) $text=$text." / "; $text=$text."<b>".$arResult['PROPERTIES']['UPAK_KG']['VALUE']." кг</b>"; }
		$text=$text."</p>";
...
<!-- Получить Скидку -->
<?$button_discount="<a href='http://prokwarti.ru/discount.php'><img src='".SITE_TEMPLATE_PATH."/images/discount.png' title='Получить скидку' alt='Получить скидку' style='width:120px; height:47px;' /></a>";?>
<?=$button_discount?>
...
<!-- Скидка -->
				...
				<?if($arDiscounts[0]["VALUE"]):?><br /><p><b class='orange' style='font-size: 20px;'><?=$arDiscounts[0]["NOTES"]?></b></p><?endif?>
<!-- Шоурум -->
<? if($type_item['UF_SHOWROOM']) $showroom="<div style='margin: 60px 0 0px 0px;'><p><a href='/about/contacts/'><img src='".SITE_TEMPLATE_PATH."/images/eye.png' style='width:30px; margin-top:0px; margin-right:10px; margin-top:-7px; float:left;' /></a>данный товар можно посмотреть в нашем <a href='/about/contacts/'>шоу-руме</a></p></div>"; ?>
<?=$showroom?>
<!-- Конец Шоурум -->
<p class="back" style="margin-top:20px;"><a href="#" onclick="history.back(); return false;">Назад</a></p>
			</td>
		</tr>
	</table>
