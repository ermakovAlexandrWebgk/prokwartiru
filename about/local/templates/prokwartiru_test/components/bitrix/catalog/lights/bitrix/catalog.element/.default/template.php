...
<?
  ...
  if($type_item = $type->GetNext()){
	...
	if($country=GetIBlockSection($fabrika['IBLOCK_SECTION_ID'], 'catalog')) $title=$title.". <span style='font-size: 80%;'>".$country['NAME']."</span>"; 
        ...
        if($arResult['PROPERTIES']['NEW']['VALUE']) $title=$title."&nbsp;&nbsp;&nbsp;<span id='new'>NEW</span>";
        if($arResult['PROPERTIES']['HIT']['VALUE']) $title=$title."&nbsp;&nbsp;&nbsp;<span id='hit'>HIT</span>";
        if($arResult['PROPERTIES']['SALE']['VALUE']) $title=$title."&nbsp;&nbsp;&nbsp;<span id='sale'>SALE</span>";
  }
  $title=$title."</h1>";
?>
<?=$title?>
...
<?
  if($arResult['PROPERTIES']['RUBRIC']['VALUE']) {
    $itemRubric = CIBlockElement::GetByID($arResult['PROPERTIES']['RUBRIC']['VALUE']); 
    if($ar_itemRubric = $itemRubric->GetNext()){
      //$text=$text."<p>Рубрика: <b>".$ar_itemRubric['NAME']."</b></p>";
      echo("<b>".$ar_itemRubric['NAME']."</b><br /><br />");
    }  
  }
?>
...
<!-- Цена Товара -->				
				...
				?>
				  ...
				  <p>Цена: <!--label>Цена:</label-->
					...
					<?endif;?>
                                                 <?if($arResult['PROPERTIES']['UNIT']['VALUE']):?>/ <?=$arResult['PROPERTIES']['UNIT']['VALUE'];?><?else:?>/ Шт.<?endif?> 
				  </p>
				<?
						break;
					endif;
...
Наличие: <?if($itemCatalog["QUANTITY"]>0):?><b class="orange">Складская программа</b><?else:?><b class="orange">Под заказ</b><?endif?>
...
<?
		...
		if($arResult['PROPERTIES']['POWER']['VALUE']) $text=$text."<p>Мощность: <b>".$arResult['PROPERTIES']['POWER']['VALUE']."</b></p>";
...
<p class="back"><a href="#" onclick="history.back(); return false;">Назад</a></p>
	
</div>

<?$APPLICATION->SetTitle($ar_itemRubric['NAME']." ".$section['NAME']." (".$fabrika['NAME'].") арт. ".$arResult['NAME']." в интернет-магазине www.prokwarti.ru");?>
