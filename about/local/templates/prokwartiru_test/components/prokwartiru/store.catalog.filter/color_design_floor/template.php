...
<table cellpadding="0"cellspacing="0" border="0">
    <tr>
        <td style="width: 150px;">Подобрать покрытия</td>
        <td>
            <select onchange="javascript: if(this.options[this.selectedIndex].value==''&&<? if(!$DESIGN_ID&&!$PRICE_ID&&!$CLASS_ID) echo('true'); else echo('false'); ?>) document.location='index.php'; else document.location='color_design.php?COLOR_ID='+this.options[this.selectedIndex].value+'&DESIGN_ID=<?=$DESIGN_ID?>'+'&PRICE_ID=<?=$PRICE_ID?>'+'&CLASS_ID=<?=$CLASS_ID?>'; ">
              <option value="" <?if($COLOR_ID==""):?>selected<?endif?> >Все по цвету</option>
<?
$db_color_list = CIBlockProperty::GetPropertyEnum("COLOR", Array("SORT"=>"asc"), Array("IBLOCK_ID"=>24));
...
?>
        ...
        <td>
            <select onchange="javascript: if(this.options[this.selectedIndex].value==''&&<? if(!$COLOR_ID&&!$PRICE_ID&&!$CLASS_ID) echo('true'); else echo('false'); ?>) document.location='index.php'; else document.location='color_design.php?COLOR_ID=<?=$COLOR_ID?>'+'&DESIGN_ID='+this.options[this.selectedIndex].value+'&PRICE_ID=<?=$PRICE_ID?>'+'&CLASS_ID=<?=$CLASS_ID?>';">
              <option value="" <?if($DESIGN_ID==""):?>selected<?endif?> >Все по фактуре</option>
<?
$db_design_list = CIBlockProperty::GetPropertyEnum("DESIGN", Array("SORT"=>"asc"), Array("IBLOCK_ID"=>24));
...
?>
        ...
        <td class="separator"></td>
        <td>
            <select onchange="javascript: if(this.options[this.selectedIndex].value==''&&<? if(!$COLOR_ID&&!$DESIGN_ID&&!$CLASS_ID) echo('true'); else echo('false'); ?>) document.location='index.php'; else document.location='color_design.php?COLOR_ID=<?=$COLOR_ID?>'+'&DESIGN_ID=<?=$DESIGN_ID?>'+'&PRICE_ID='+this.options[this.selectedIndex].value+'&CLASS_ID=<?=$CLASS_ID?>';">
        ...
        <td class="separator"></td>
        <td>
            <select onchange="javascript: if(this.options[this.selectedIndex].value==''&&<? if(!$COLOR_ID&&!$DESIGN_ID&&!$PRICE_ID) echo('true'); else echo('false'); ?>) document.location='index.php'; else document.location='color_design.php?COLOR_ID=<?=$COLOR_ID?>'+'&DESIGN_ID=<?=$DESIGN_ID?>'+'&PRICE_ID=<?=$PRICE_ID?>'+'&CLASS_ID='+this.options[this.selectedIndex].value;">
              <option value="" <?if($CLASS_ID==""):?>selected<?endif?> >Все классы</option>
<?
$db_class_list = CIBlockProperty::GetPropertyEnum("CLASS", Array("SORT"=>"asc"), Array("IBLOCK_ID"=>24));
while($ar_class_list = $db_class_list->GetNext())
{
?>
              <option value="<?=$ar_class_list["ID"]?>" <?if($CLASS_ID==$ar_class_list["ID"]):?>selected<?endif?> ><?=$ar_class_list["VALUE"]?></option>
<?
}
?>
            </select>
