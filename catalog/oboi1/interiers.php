...
<!-- Фильтр -->
 <?if((($arIBlockSection['IBLOCK_SECTION_ID']=="") && ($_REQUEST['SECTION_ID'])) || ($_REQUEST['FABRIKA_ID'])):?> <?$APPLICATION->IncludeComponent("prokwartiru:store.catalog.filter", "oboi_filters", array(
	"IBLOCK_TYPE" => "catalog",
	"IBLOCK_ID" => "5",
...
);?>  <?elseif(!$_REQUEST['SECTION_ID']):?> <?$APPLICATION->IncludeComponent("prokwartiru:store.catalog.filter", "oboi_filters", array(
	"IBLOCK_TYPE" => "catalog",
	"IBLOCK_ID" => "5",
	"FILTER_NAME" => "arFilterDop",
	"FIELD_CODE" => array(
...
<!-- ЕСЛИ НЕ ГЛАВНАЯ СТРАНИЦА РАЗДЕЛА ОБОИ (тяжелый вариант) -->
 <?$APPLICATION->IncludeComponent("bitrix:catalog", "oboi_interiers", Array(
	...
	"SEF_FOLDER" => "/catalog/oboi/",	// Каталог ЧПУ (относительно корня сайта)
