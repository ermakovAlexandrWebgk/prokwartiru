	...
	for($i = 0; $i < $arParams["NUM_CATEGORIES"]; $i++)
	{
		...
		if ($isSearchInstalled)
		{
            ...
            $hlblockResultData = $hlblockObj->getElements(
                ...
                foreach ($hlblockResultData as $curHlblockResult) {
                    $arResult["CATEGORIES"][$i]["ITEMS"][$curHlblockResult["UF_ITEM_ID"]] = array(
                        "NAME" => $curHlblockResult["UF_ITEM_NAME"],
                        "URL" => htmlspecialcharsbx($curHlblockResult["UF_ITEM_URL"]),
                        "ITEM_ID" => $curHlblockResult["UF_ITEM_ID"],
                        "MODULE_ID" => "iblock"
                    );
                }
            if (count($arResult["CATEGORIES"][$i]["ITEMS"]) < $arParams["TOP_COUNT"]) {
                $pattern = "/(\W)/";
                $query = strtolower(preg_replace($pattern, "", $query));
                $hlblockResultData = $hlblockObj->getElements(
                    array(
                        'filter' => array(
                                "%UF_ARTICLE" => $query
