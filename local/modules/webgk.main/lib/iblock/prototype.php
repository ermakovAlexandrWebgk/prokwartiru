    ...
    */
    class Prototype
    {
        ...
        */
        ...
        /**
        * функция генерации символьного кода для элемента/раздела
        *
        * @param mixed $arFields
        */
        public function setElementCode(&$arFields) { 
            
            $name = $arFields["NAME"];

            if (!$name && $arFields["ID"]) {
                $item = \CIBlockElement::GetList(array(), array("ID" => $arFields["ID"], "IBLOCK_CODE" => "catalog_1c_catalog"), false, false, array("NAME"))->Fetch();
                $name = $item["NAME"];
            }

            if ($name) {
                $arFields["CODE"] = UTil::str2url($name, "ru");
                //$arFields["CODE"] = \CUTil::translit($name, "ru");
                tools::arshow($arFields);
            }                                
        ...
        * @param mixed $arFields
        */
        public function setSectionCode(&$arFields) { 
            
            $name = $arFields["NAME"];

            if (!$name && $arFields["ID"]) {
                $item = \CIBlockSection::GetList(array(), array("ID" => $arFields["ID"], "IBLOCK_CODE" => "catalog_1c_catalog"), false, array("NAME"))->Fetch();
                $name = $item["NAME"];
            }
