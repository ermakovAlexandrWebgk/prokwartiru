        ...
        if ($arIBlock = $rsIBlocks->Fetch())
        {
            ...
            if ($bRightBlock)
            {
                ...
                while ($ar_section = $db_section->Fetch())
                {
                    ...
                    {
                        if($ar_section["ID"] != 34609){?>window.parent.Tree[<?echo intval($ar_section["ID"]);?>]=new Array();<?}
                    }
                        if($ar_section["ID"] != 34609){?>window.parent.Tree[<?echo intval($ar_section["IBLOCK_SECTION_ID"]);?>][<?echo intval($ar_section["ID"]);?>]=Array('<?echo CUtil::JSEscape(htmlspecialcharsbx($ar_section["NAME"]));?>', '');<?}
                }
            }
        }
    }
