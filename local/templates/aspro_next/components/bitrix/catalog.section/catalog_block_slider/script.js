...
window.JCCatalogSection.prototype.setPrice = function(price, measure)
{
	...
	if (!!this.obPrice){
		strPrice = '<span class="values_wrapper">'+getCurrentPrice(price.DISCOUNT_VALUE, price.CURRENCY, price.PRINT_DISCOUNT_VALUE)+'</span>';
		if(measure){
		...
		{
			$(this.obPriceOld).find('strike').show().html(getCurrentPrice(price.VALUE, price.CURRENCY, price.PRINT_VALUE));
			if(this.showPercent){
