<?if (!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();?>
<? $this->setFrameMode( true ); ?>
<?if( !empty( $arResult ) ){
	global $arTheme;?>
	<ul class="menu top menu_top_block catalogfirst">
		<?foreach( $arResult as $key => $arItem ){?>
			<li class="catalog icons_fa <?=($arItem["CHILD"] ? "has-child" : "");?> <?=($arItem["SELECTED"] ? "current" : "");?>">
				<a class="<?=($arItem["CHILD"] ? "parent" : "");?>" href="<?=$arItem["SECTION_PAGE_URL"]?>" ><span><?=$arItem["NAME"]?></span></a>
				<?if($arItem["CHILD"]){?>
					<ul class="dropdown">
						<?foreach($arItem["CHILD"] as $arChildItem){?>
							<li class="full <?=($arChildItem["CHILD"] ? "has-child" : "");?> <?if($arChildItem["SELECTED"]){?> current <?}?> m_<?=strtolower($arTheme["MENU_POSITION"]["VALUE"]);?>">
								<a class="icons_fa <?=($arChildItem["CHILD"] ? "parent" : "");?>" href="<?=$arChildItem["SECTION_PAGE_URL"];?>"><?=$arChildItem["NAME"];?></a>
								<?if($arChildItem["CHILD"]){?>
									<ul class="dropdown">
										<?foreach($arChildItem["CHILD"] as $arChildItem1){?>
											<li class="menu_item <?if($arChildItem1["SELECTED"]){?> current <?}?>">
												<?if($arChildItem1["IMAGES"]){?>
													<span class="image"><a href="<?=$arChildItem1["SECTION_PAGE_URL"];?>"><img src="<?=$arChildItem1["IMAGES"]["src"];?>" /></a></span>
												<?}?>
												<a class="section" href="<?=$arChildItem1["SECTION_PAGE_URL"];?>"><span><?=$arChildItem1["NAME"];?></span></a>
												<?if($arChildItem1["CHILD"]){?>
													<ul class="dropdown">
														<?foreach($arChildItem1["CHILD"] as $arChildItem2){?>
															<li class="menu_item <?if($arChildItem2["SELECTED"]){?> current <?}?>">
																<a class="section1" href="<?=$arChildItem2["SECTION_PAGE_URL"];?>"><span><?=$arChildItem2["NAME"];?></span></a>
															</li>
														<?}?>
													</ul>
												<?}?>
												<div class="clearfix"></div>
											</li>
										<?}?>
									</ul>
								<?}?>
							</li>
						<?}?>
					</ul>
				<?}?>
			</li>
		<?}?>
	</ul>
<?}?>
<style>
li.icons_fa.has-child:hover .dropdown {
    display: block;
}
.menu.menu_top_block {
    background: inherit;
    white-space: normal !important;
}
li.icons_fa a {
    border: 1px solid #f2f2f2;
    border-bottom-width: 0px;
    background: #f8f8f9;
    position: relative;
    display: block;
    padding: 11px 31px 13px 17px;
    background: #f9f9f9;
    text-decoration: none;
    text-align: left;
    color: #333333;
    font-size: 13px;
    white-space: normal;
}
.menu_top_block li.catalog > a:before {
    display: none;
}
.menu_top_block li.has-child>a:after {
    font-size: 13px;
    top: 19px;
    text-align: right;
    color: #888;
    content: "\f107"; 
}
li.icons_fa a:after {
    content: "\f107";
    color: #fff;
    position: absolute;
    right: 10px !important;
    top: 50%;
    margin: -3px 0 0 0px;
    width: 12px !important;
    height: 8px !important;
    font-weight: normal;
    display: block !important;
    font: normal normal normal 14px/1 FontAwesome;
    font-size: inherit;
    opacity: 1 !important;
    background: inherit !important;
}
.menu_top_block li.catalog>a {
    padding-right: 36px;
}
.right_block ul li {
    margin: 0;
    padding-left: 0;
}
.menu_top_block li.full.m_line {
    width: 100%;
}
.menu_top_block li.catalog > a {
    padding-left: 34px;
}
</style>
<?if ($_REQUEST["current_section"] == 6362) {?>
    <style>
    .menu_top_block li.catalog > a {
        padding-left: 32px;
        padding-right: 34px;
    }
    </style>
    
<?}?>