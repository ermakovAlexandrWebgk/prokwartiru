...
?>
					<td><a class="compare-delete-item" href="<?=htmlspecialchars($APPLICATION->GetCurPageParam("action=DELETE_FROM_COMPARE_RESULT&IBLOCK_ID=".$arParams['IBLOCK_ID']."&ID[]=".$arElement['ID'],array("action", "IBLOCK_ID", "ID")))?>" title="<?=GetMessage("CATALOG_REMOVE_PRODUCT")?>"></a>Àðò. <a href="<?=$arElement['DETAIL_PAGE_URL']?>"><?=$arElement['NAME']?></a></td>
<?
endforeach;
?>
				</tr>
...
?>
						<a href="<?=$arElement['DETAIL_PAGE_URL']?>"><img border="0" src="<?=$arElement['FIELDS'][$code]['SRC']?>" width="100px<!--?=$arElement['FIELDS'][$code]['WIDTH']?-->" height="100px<!--?=$arElement['FIELDS'][$code]['HEIGHT']?-->" alt="<?=$arElement['FIELDS'][$code]['ALT']?>" /></a>
<?
				endif;
			break;
			default:
