...
?>
	<div style="background-color: #B7B6B2;" class="catalog-section<?=$bHasPicture ? '' : ' no-picture-mode'?>" id="<?=$this->GetEditAreaId($arSection['ID']);?>">
		...
		<?if ($arSection['NAME'] && $arResult['SECTION']['ID'] != $arSection['ID']):?>
			<div class="catalog-section-title" style="background-color: #B7B6B2; text-transform: uppercase;"><a style="color:#302f2a; text-decoration: none; font-weight: bold;" href="<?=$arSection["SECTION_PAGE_URL"]?>"><?=$arSection["NAME"]?></a></div>
		<?endif;?>
		...
		<?if ($bHasChildren):?>
			<div class="catalog-section-childs" style="background-color: #B7B6B2;">
				<table cellspacing="20" class="catalog-section-childs">
				<?
				$cell = 0;
