	...
	<div class="catalog-interier-card">
		<div class="item-title" style="text-align: center;"><?=$arInteriers["NAME"]?><?=$sticker?></div>
  		<div class="item-image">
			<a rel="detail-images" href="<?=$arInteriersPathDetail?>" title="<div id='interier_name'><?=$arInteriers['NAME']?></div>"><img src="<?=$arInteriersPathDetail?>" alt="<?=$arInteriers['NAME']?>" title="Открыть картинку" id="catalog_list_image_<?=$arInteriers['ID']?>" style="width: 125px; height: 125px;" /></a>
		</div>
	  </div>
    <? }?>
  </div>
...
<? if(count($arResult['ITEMS'])< 1) {
	//if(count($Interiers)==1) echo("Каталог пока не заполнен");
	return;
}
?>
...
<p style="padding: 5px 5px 10px 10px; text-align: center; font-size: 13px;"><i>Для получения подробной информации о товаре кликните на картинку.</i></p>
...
<div class="item-image" style="width: 145px; height: auto;">
	<a rel="detail-images" /*onclick="javascript: ShowFloatCard(<-?=$arElement['DETAIL_PICTURE']['WIDTH']?->, <-?=$arElement['DETAIL_PICTURE']['HEIGHT']?->);"*/ href="<?if($arElement['DETAIL_PICTURE']['SRC']):?><?=$arElement['DETAIL_PICTURE']['SRC']?><?else:?><?=SITE_TEMPLATE_PATH?>/images/default300.gif<?endif?>" title="<?=$title_text?>"><img src="<?if($arElement['PREVIEW_IMG']['SRC']):?><?=$arElement['PREVIEW_IMG']['SRC']?><?else:?><?=SITE_TEMPLATE_PATH?>/images/default100.gif?><?endif?>" style="max-width: 145px; max-height: 145px; width: <?=$arElement['PREVIEW_IMG']['WIDTH']?>; height: <?=$arElement['PREVIEW_IMG']['WIDTH']?>;" alt="<?=$arElement["NAME"]?>" title="Открыть картинку" id="catalog_list_image_<?=$arElement['ID']?>"/></a>
</div>
<!-- Конец Картинка -->
<!--?endif;?-->
