...
<?if($path_pic_detail):?>
      <div class="catalog-interier-card">
       <!-- Заголовок -->
       <? if($path) $text_title="<a href='".$path."index.php?SECTION_ID=".$interierList[$i]['ID']."'><b><nobr>".$interierList[$i]['NAME']."</nobr></b></a>"; else $text_title="<b><nobr>".$interierList[$i]['NAME']."</nobr></b>";?>
	<div class="item-title" style="text-align: center;"><?=$text_title?><?=$sticker?></div>
	<!-- Конец Заголовок -->
	<?$numberElements++;?>
        <div class="item-image">
	    <a rel="catalog-detail-images" href="<?=$path_pic_detail?>" title=""><img src="<?=$path_pic_detail?>" width="125px" height="125px" alt="" title="" id="catalog_list_image_<?=$i?>" /></a>
        </div>
