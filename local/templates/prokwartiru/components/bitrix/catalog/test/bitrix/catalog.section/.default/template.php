...
<?
  ...
  if($section) $type = CIBlockSection::GetList(array(), array('IBLOCK_ID' => $iblock_ID, 'ID' => $section['ID']), false, $arSelect=array('UF_FABRIKA','UF_NEWCATALOG', 'UF_HIT','UF_SALE','UF_GLUE','UF_MARK'));
...
<?if($glue):?>
  <div id="glue"> 
    <p><a href="/catalog/glue/?ELEMENT_ID=<?=$glue['ID']?>"><?if($glue_pic):?><img src="<?=$glue_pic?>"> <?endif?>С этими обоями фабрика рекомендует клей KLEO</a></p>
  </div>
<?endif?>
<!-- Конец Клей -->
	...
	<div class="catalog-interier-card">
		<div class="item-title" style="text-align: center;"><nobr><?=$arInteriers["NAME"]?></nobr><?=$sticker?></div>
  		<div class="item-image">
			<a rel="detail-images" href="<?=$arInteriersPathDetail?>" title="<div id='interier_name'><?=$arInteriers['NAME']?></div>"><img src="<?=$arInteriersPathPreview?>" alt="<?=$arInteriers['NAME']?>" title="Открыть картинку" id="catalog_list_image_<?=$arInteriers['ID']?>" width="125" height="125" /></a>
		</div>
	  </div>
    <? }?>
  </div>
...
<a class="fancybox-buttons" data-fancybox-group="button" href="<?=$arElement['DETAIL_PICTURE']['SRC']?>"><img src="<?=$arElement['PREVIEW_IMG']['SRC']?>" alt="" /></a>
</div>
<!-- Конец Картинка -->
<!--?endif;?-->
