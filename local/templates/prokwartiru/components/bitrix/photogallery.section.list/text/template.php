...
?>
	<li style="padding: 5px; /*10px;*/" class="photo-album-item photo-album-<?=($res["ACTIVE"] != "Y" ? "nonactive" : "active")?> <?=(
...
<!-- Конец Обложка каталога -->

<!-- Название Альбома -->
<div class="photo-album-name">
							<a href="<?=$res["LINK"]?>" id="photo_album_name_<?=$res["ID"]?>" title="<?=htmlspecialchars($res["~NAME"])?>" <?
								?> onmouseover="__photo_check_name_length(event, this);"><span style="font-weight: normal;"><?=$res["NAME"]?></span></a>&nbsp;&nbsp;&nbsp;\</div>
<!-- Конец Название Альбома -->
