...
while($ar_design_list = $db_design_list->GetNext()) $design=$ar_design_list["VALUE"];
  $page_title="<h1 id='page_title' style='padding-left: 10px;'><span>Напольные покрытия";
  ...
  $window_title="Напольные покрытия";
  $keywords="напольные покрытия ".$color.", напольные покрытия ".$design;
...
// ФИЛЬТР
$arFilterMain=Array('ACTIVE' => 'Y', "IBLOCK_ID"=>$iblock_ID, "PROPERTY_COLOR"=>$COLOR_ID, "PROPERTY_DESIGN"=>$DESIGN_ID, "PROPERTY_CLASS"=>$CLASS_ID);
//global $arFilterDop;
...
<? $section=GetIBlockSection($arElement["IBLOCK_SECTION_ID"], 'catalog');
  if($section) $type = CIBlockSection::GetList(array(), array('IBLOCK_ID' => $iblock_ID, 'ID' => $section['ID']), false, $arSelect=array('UF_FABRIKA','UF_NEW'));
	...
	if($ar_element = $element->GetNext())
		{
		  ...
		  while ($ar_property=$property->GetNext()){
                      if($ar_property['VALUE']){
                        if($ar_property['CODE']=='PROPERTY') $text=$text."Основные характеристики: ".$ar_property['VALUE']."<br />";
		        if($ar_property['CODE']=='SIZE') $text=$text."Размер: ".$ar_property['VALUE']."<br />";
		        if($ar_property['CODE']=='DESIGN') {
                          $props = CIBlockProperty::GetPropertyEnum("DESIGN", Array("SORT"=>"asc"), Array("IBLOCK_ID"=>24, "ID"=>$ar_property['VALUE']));
                          if($prop = $props->GetNext()) $text=$text."Фактура: ".$prop['VALUE']."<br />";
                        }
		        if($ar_property['CODE']=='COLOR'){
                          $props = CIBlockProperty::GetPropertyEnum("COLOR", Array("SORT"=>"asc"), Array("IBLOCK_ID"=>24, "ID"=>$ar_property['VALUE']));
                          if($prop = $props->GetNext()) $text=$text."Цвет: ".$prop['VALUE']."<br />";
                        } 
		        if($ar_property['CODE']=='CLASS'){
                          $props = CIBlockProperty::GetPropertyEnum("CLASS", Array("SORT"=>"asc"), Array("IBLOCK_ID"=>24, "ID"=>$ar_property['VALUE']));
                          if($prop = $props->GetNext()) $text=$text."Класс: ".$prop['VALUE']."<br />";
                        }  
		        if($ar_property['CODE']=='COVER'){
                          $props = CIBlockProperty::GetPropertyEnum("COVER", Array("SORT"=>"asc"), Array("IBLOCK_ID"=>24, "ID"=>$ar_property['VALUE']));
                          if($prop = $props->GetNext()) $text=$text."Покрытие: ".$prop['VALUE']."<br />";
                        } 
		        if($ar_property['CODE']=='LOCK'){
                          $props = CIBlockProperty::GetPropertyEnum("LOCK", Array("SORT"=>"asc"), Array("IBLOCK_ID"=>24, "ID"=>$ar_property['VALUE']));
                          if($prop = $props->GetNext()) $text=$text."Замковая система: ".$prop['VALUE']."<br />";
                        } 
		        if($ar_property['CODE']=='SURFACE'){
                          $props = CIBlockProperty::GetPropertyEnum("SURFACE", Array("SORT"=>"asc"), Array("IBLOCK_ID"=>24, "ID"=>$ar_property['VALUE']));
                          if($prop = $props->GetNext()) $text=$text."Поверхность: ".$prop['VALUE']."<br />";
                        } 
		        if($ar_property['CODE']=='PERIOD'){
                          $props = CIBlockProperty::GetPropertyEnum("PERIOD", Array("SORT"=>"asc"), Array("IBLOCK_ID"=>24, "ID"=>$ar_property['VALUE']));
                          if($prop = $props->GetNext()) $text=$text."Срок службы: ".$prop['VALUE']."<br />";
                        } 
		        if($ar_property['CODE']=='LAMEL'){
                          $props = CIBlockProperty::GetPropertyEnum("LAMEL", Array("SORT"=>"asc"), Array("IBLOCK_ID"=>24, "ID"=>$ar_property['VALUE']));
                          if($prop = $props->GetNext()) $text=$text."Количество полос: ".$prop['VALUE']."<br />";
                        } 
		        if($ar_property['CODE']=='FASKA'){
                          $props = CIBlockProperty::GetPropertyEnum("FASKA", Array("SORT"=>"asc"), Array("IBLOCK_ID"=>24, "ID"=>$ar_property['VALUE']));
                          if($prop = $props->GetNext()) $text=$text."Фаска: ".$prop['VALUE']."<br />";
                        }
                        if($ar_property['CODE']=='UPAK_KBM'&&$ar_property['VALUE']) $text=$text."Упаковка: ".$ar_property['VALUE']." кв.м";
                        if($ar_property['CODE']=='UPAK_PCS'&&$ar_property['VALUE']) $text=$text." / ".$ar_property['VALUE']." шт.";
                        if($ar_property['CODE']=='UPAK_KG'&&$ar_property['VALUE']) $text=$text." / ".$ar_property['VALUE']." кг";
...
<?
	if (is_array($arElement['DISPLAY_PROPERTIES']) && count($arElement['DISPLAY_PROPERTIES']) > 0){
		...
		if($arElement['PROPERTIES']['DESIGN']['VALUE']) $text=$text."Фактура: ".$arElement['PROPERTIES']['DESIGN']['VALUE']."<br />";
		if($arElement['PROPERTIES']['COLOR']['VALUE']) $text=$text."Цвет: ".$arElement['PROPERTIES']['COLOR']['VALUE']."<br />";
		if($arElement['PROPERTIES']['CLASS']['VALUE']) $text=$text."Класс: ".$arElement['PROPERTIES']['CLASS']['VALUE']."<br />";
		if($arElement['PROPERTIES']['COVER']['VALUE']) $text=$text."Покрытие: ".$arElement['PROPERTIES']['COVER']['VALUE']."<br />";
		if($arElement['PROPERTIES']['LOCK']['VALUE']) $text=$text."Замковая система: ".$arElement['PROPERTIES']['LOCK']['VALUE']."<br />";
		if($arElement['PROPERTIES']['SURFACE']['VALUE']) $text=$text."Поверхность: ".$arElement['PROPERTIES']['SURFACE']['VALUE']."<br />";
		if($arElement['PROPERTIES']['PERIOD']['VALUE']) $text=$text."Срок службы: ".$arElement['PROPERTIES']['PERIOD']['VALUE']."<br />";
		if($arElement['PROPERTIES']['LAMEL']['VALUE']) $text=$text."Количество полос: ".$arElement['PROPERTIES']['LAMEL']['VALUE']."<br />";
		if($arElement['PROPERTIES']['FASKA']['VALUE']) $text=$text."Фаска: ".$arElement['PROPERTIES']['FASKA']['VALUE']."<br />";


		//if($arElement['PROPERTIES']['RAPPORT']['VALUE']) $text=$text."Раппорт (подбор): ".$arElement['PROPERTIES']['RAPPORT']['VALUE']."</p>";

		if($arElement['PROPERTIES']['UPAK_KBM']['VALUE']||$arElement['PROPERTIES']['UPAK_PCS']['VALUE']||$arElement['PROPERTIES']['UPAK_KG']['VALUE']) $text=$text."Упаковка: ";
		...
		if($arElement['PROPERTIES']['UPAK_KG']['VALUE']) { if($arElement['PROPERTIES']['UPAK_PCS']['VALUE']) $text=$text." / "; $text=$text.$arElement['PROPERTIES']['UPAK_KG']['VALUE']." кг"; }
...
<?$APPLICATION->SetPageProperty("keywords", "напольные покрытия, каталог напольных покрытий, ".$keywords.", ".$keywords." цена, ".$keywords." купить");?>