...
<?
  ...
  $arSelect=array('UF_FABRIKA','UF_NEWCATALOG', 'UF_HIT', 'UF_DISCOUNT10','UF_SALE','UF_GLUE','UF_MARK', 'UF_SALE_OBOI', 'UF_ACTION');
...
<? $interiersNumber=10; if($section['ID']==842||$section['ID']==1812||$section['ID']==3730) $interiersNumber=1000; ?>
<?if($section){
...
<?if($Interiers):?>
  ...
  ?>
	<div class="catalog-interier-card">
		<div class="item-title" style="text-align: center; font-size: 12px;"><nobr><?=$arInteriers["NAME"]?></nobr><?=$sticker?></div>
  		<div class="item-image">
			<!--a class="fancybox" data-fancybox-group="gallery" href="#inline_<-?=$arInteriers['ID']?->"><img src="<-?if($arInteriersPathPreview):?-><-?=$arInteriersPathPreview?-><-?else:?-><-?=SITE_TEMPLATE_PATH?->/images/default125.gif<-?endif?->" alt="<-?=$arInteriers['NAME']?->" title="Открыть картинку" id="catalog_list_image_<-?=$arInteriers['ID']?->" style="<-?if($arElement['PREVIEW_IMG']['WIDTH']>$arElement['PREVIEW_IMG']['HEIGHT']):?->height:auto;<-?else:?->width:auto;<-?endif?->" /></a-->
			<a class="fancybox" data-fancybox-group="gallery" href="#inline_<?=$arInteriers['ID']?>"><img src="<?if($arInteriersPathPreview):?><?=$arInteriersPathPreview?><?else:?><?=SITE_TEMPLATE_PATH?>/images/default125.gif<?endif?>" alt="<?=$arInteriers['NAME']?>" title="Открыть картинку" id="catalog_list_image_<?=$arInteriers['ID']?>" style="<?if($arElement['PREVIEW_IMG']['WIDTH']>$arElement['PREVIEW_IMG']['HEIGHT']):?>height:125px;<?else:?>width:125px;<?endif?>" /></a>
...
?>
	...
	<div class="catalog-item-card" style="height: 160px;">


<!-- Заголовок -->
  <div class="item-title" style="height:13px;"><span>Арт.</span> <a href="<?=$arElement['DETAIL_PAGE_URL']?>" title="<?=$arElement['NAME']?>"><b><?=$arElement["NAME"]?></b></a><?=$sticker?></div>
<!-- Конец Заголовок -->
...
<!--?
	...
	if ($arElement['CAN_BUY']) $button_buy=$button_buy."<div class='buttons_'><a href='".$arElement['ADD_URL']."' class='catalog-item-buy' rel='nofollow'  onclick='return addToCart(this, 'catalog_list_image_".$arElement['ID']."', 'list', '".GetMessage('CATALOG_IN_CART')."');' id='catalog_add2cart_link_".$arElement['ID']."'>".GetMessage('CATALOG_ADD')."</a></div>";
...
<!-- Формирование и форматирование Описания для всплывающей карточки товара -->
	...
	<?$title_text="<table cellspacing='0' cellpadding='0' border='0' class='float-card'><tr><td class='left' valign='bottom'>".$button_buy."</td></tr><tr><td class='left'><p>Артикул: <b class='big'>".$title."</b></p><!--p>Цена: ".$price."</p--><p>Наличие: <b class='orange'>".$quantity."</b></p></td></tr><tr><td>".$text."</td></tr><tr><td>".$discount."<p style='margin: 10px 0 0 -30px;'>".$delivery."</p></td></tr></table>";?>
...
<!-- Картинка -->
<div class="item-image" style="padding: 0 6px 3px 6px;">
