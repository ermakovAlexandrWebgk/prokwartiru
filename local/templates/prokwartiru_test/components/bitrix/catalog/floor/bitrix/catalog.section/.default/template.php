...
<?
  ...
  $arSelect=array('UF_FABRIKA','UF_NEWCATALOG', 'UF_HIT', 'UF_DISCOUNT10','UF_SALE','UF_GLUE','UF_MARK', 'UF_SALE_FLOOR', 'UF_ACTION', 'UF_SHOWROOM');
...
$saleID="";
$saleID=$type_item['UF_SALE_FLOOR'];
//echo($saleID);
if($saleID){
  ...
  //if($countryName=="Не указана") $countryName="";
        ...
        if($type_item['UF_SALE_FLOOR'][0]) $page_title=$page_title."<span id='sale'>SALE ".$sale."</span>&nbsp;";
        elseif($type_item['UF_ACTION']) $page_title=$page_title."<span id='sale'style='background-color:#ffce0c;'>ЛУЧШАЯ ЦЕНА</span>&nbsp;";
        elseif($type_item['UF_NEWCATALOG']) $page_title=$page_title."<span id='new'>NEW</span>&nbsp;";
        elseif($type_item['UF_HIT']) $page_title=$page_title."<span id='hit'>HIT</span>&nbsp;";
	...
	if($country=GetIBlockSection($fabrika['IBLOCK_SECTION_ID'], 'catalog')) $page_title=$page_title.". <span style='font-size: 80%;'>".$country['NAME']."</span>"; 
        if(($fabrika['NAME']=="Loymina")||($fabrika['NAME']=="Seabrook")) { $delivery="<a href='/about/delivery/'><img src='".SITE_TEMPLATE_PATH."/images/free.png' title='Бесплатная доставка' alt='Бесплатная доставка'  style='width:145px; height:30px; margin-bottom:-5px; margin-left:30px;' /></a>"; $page_title=$page_title.$delivery;}
 }
...
<!-- Шоурум -->
<? if($type_item['UF_SHOWROOM']) $showroom="<div style='margin: 60px 0 0px 0px;'><p><a href='/about/contacts/'><img src='".SITE_TEMPLATE_PATH."/images/eye.png' style='width:30px; margin-top:0px; margin-right:10px; float:left;' /></a>данный товар можно посмотреть<br />в нашем <a href='/about/contacts/'>шоу-руме</a></p></div>"; ?>
<!-- Конец Шоурум -->
...
<? $interiersNumber=10; if($section['ID']==842||$section['ID']==1812) $interiersNumber=1000; ?>
<?if($section){
...
<!-- Заголовок -->
  <div class="item-title" style="height:14px;"><!--span>Арт.</span--> <a href="<?=$arElement['DETAIL_PAGE_URL']?>" title="<?=$arElement['NAME']?>"><b><?=$arElement["NAME"]?></b></a><?=$sticker?></div>
<!-- Конец Заголовок -->
...
<?
	if (is_array($arElement['DISPLAY_PROPERTIES']) && count($arElement['DISPLAY_PROPERTIES']) > 0){
		...
		//if($arElement['PROPERTIES']['RAPPORT']['VALUE']) $text=$text."Раппорт (подбор): ".$arElement['PROPERTIES']['RAPPORT']['VALUE']."<br />";
		...
		if($arElement['PROPERTIES']['UPAK_KBM']['VALUE']||$arElement['PROPERTIES']['UPAK_PCS']['VALUE']||$arElement['PROPERTIES']['UPAK_KG']['VALUE']) $text=$text."<br />";
	
		//if($arElement["DETAIL_PAGE_URL"]) $text=$text."<p><a href='".$arElement["DETAIL_PAGE_URL"]."'>Подробнее</a></p>";	
		$text=$text.$quantity."</p></div>";
	}
?>
...
<!-- Формирование и форматирование Описания для всплывающей карточки товара -->
	...
	<?$title_text="<table cellspacing='0' cellpadding='0' border='0' class='float-card'><tr><td valign='top' style='min-width: 400px;'><p>Название: <b class='big'>".$title."</b></p><p>Цена: ".$price."</p></td></tr><tr><td>".$text."</td></tr><tr><td>".$discount."<p style='margin: 10px 0 0 -30px;'>".$delivery."</p></td></tr><tr><td valign='top' style='min-width: 300px;'>".$button_buy.$button_discount.$showroom."</td></tr></table>";?>
...
<?endif?>
<?$APPLICATION->SetTitle("Паркет ".$window_title." в интернет-магазине www.prokwarti.ru");?>
