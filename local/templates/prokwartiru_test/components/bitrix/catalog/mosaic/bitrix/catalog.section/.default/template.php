...
<?
  $page_title="<h1 id='page_title' style='margin-top: 5px;'><span>".$section["NAME"]."</span>";
  $arSelect=array('UF_FABRIKA','UF_NEWCATALOG', 'UF_HIT','UF_SALE', 'UF_SHOWROOM');
  ...
  if($type_item = $type->GetNext()){
	...
	if($country=GetIBlockSection($fabrika['IBLOCK_SECTION_ID'], 'catalog')) $page_title=$page_title.". <span style='font-size: 80%;'>".$country['NAME']."</span>"; 
        ...
        if(($fabrika['NAME']=="Loymina")||($fabrika['NAME']=="Seabrook")) $page_title=$page_title." <span style='font-size: 12px;'>&nbsp;&nbsp;&mdash;&nbsp;Бесплатная доставка</span>";
 }
...
<!-- Заголовок -->
  <div class="item-title" style="height:15px;"><span>Арт.</span> <a href="<?=$arElement['DETAIL_PAGE_URL']?>" title="<?=$arElement['NAME']?>"><b><?=$arElement["NAME"]?></b></a><?=$sticker?></div>
<!-- Конец Заголовок -->
...
<!-- Формирование и форматирование Описания для всплывающей карточки товара -->
	...
	<?$title_text="<table cellspacing='0' cellpadding='0' border='0' class='float-card' style='min-width:400px;'><tr><td class='left'><p>Артикул: <b class='big'>".$title."</b></p><p>Цена: ".$price."</p></td></tr><tr><td>".$text."</td></tr><tr><td>".$button_buy.$button_discount.$showroom."</td></tr></table>";?>
...
<script language="JavaScript">document.write('<div id="inline_<?=$arElement['ID']?>" style="width:'+winW+'px; height:'+winH+'px; display:'+display+'; text-align: center;">');</script>
		...
		<table><tr><td valign="top" style="width: 560px; text-align:right;">
