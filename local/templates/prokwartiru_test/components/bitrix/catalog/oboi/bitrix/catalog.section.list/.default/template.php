...
<?
	...
	$filter_meter=$_REQUEST["METER"]; 
	$filter_cheap=$_REQUEST["CHEAP"]; 
	$numberFilter=0;
	$IBLOCK_ID=1;
...
<!-- Список каталогов Метровых обоев -->
<?elseif($filter_meter):?>
<!--h1 id='page_title' style='margin-left: 10px;'><span>Метровые обои на флизелине</span></h1-->
<?
// ФИЛЬТР
$arFilterMain=Array('ACTIVE' => 'Y', 'IBLOCK_ID' => $IBLOCK_ID, 'UF_METER' => true); 
$arFilterDop=Array();
if($section&&$section!=1) $arFilterDop=array_merge($arFilterDop, Array('SECTION_ID'=>$section));
if($filter_price) $arFilterDop=array_merge($arFilterDop, Array('UF_PRICE'=>$filter_price));
if($filter_width) $arFilterDop=array_merge($arFilterDop, Array('UF_WIDTH'=>$filter_width));
...
<?
    ...
    $listCatalogMeter=CIBlockSection::GetList(array('name'=>'asc'), $arFilter, false, $arSelect=array('UF_FABRIKA', 'UF_NEWCATALOG', 'UF_HIT', 'UF_SALE', 'UF_DISCOUNT10', 'UF_DISCOUNT5', 'UF_SALE_OBOI', 'UF_ACTION')); //'left_margin'=>'asc'
    while($itemCatalogMeter=$listCatalogMeter->GetNext()) 
    {
      $numberFilter++;
$saleID="";
$sale="";
$saleID=$itemCatalogMeter['UF_SALE_OBOI'];
if($saleID){
  $res = CUserFieldEnum::GetList(array(), array("ID" => $saleID, "FIELD_NAME" => "UF_SALE_OBOI"));
    if($ar_res = $res->GetNext())
      $sale=$ar_res["VALUE"];
  ...
  ?>
	<div class="catalog-item">
	  <div class="catalog-item-image"><a href="<?=$itemCatalogMeter['SECTION_PAGE_URL']?>"><?if($itemCatalogMeter["PICTURE"]):?><? echo(ShowImage($itemCatalogMeter["PICTURE"], 150, 150, "border='0' title='Открыть каталог'", "", true));?><?else:?><img src="<?=SITE_TEMPLATE_PATH?>/images/default150.gif" width="150px" height="150px" title="Открыть каталог"><?endif?></a></div>
	  <div class="catalog-item-title"><a href="<?=$itemCatalogMeter['SECTION_PAGE_URL']?>"><nobr><b><?=$itemCatalogMeter["NAME"]?></b></nobr></a></div>	  
	  <div  class="catalog-item-title" style="font-size: 13px;">
	    <?
		if($itemCatalogMeter["IBLOCK_SECTION_ID"]) $type=CIBlockSection::GetByID($itemCatalogMeter["IBLOCK_SECTION_ID"]);
		  if($ar_type=$type->GetNext())  echo $ar_type['NAME'];
	    ?>
	  </div>
	  <div style='position: relative; top: -175px; left: -3px; float:left;'><? if($sale) echo("<span id='sale'>SALE ".$sale."</span>");  elseif($itemCatalogMeter["UF_ACTION"]) echo("<span id='sale' style='background-color:#ffce0c;'>ЛУЧШАЯ ЦЕНА</span>"); elseif($itemCatalogMeter["UF_DISCOUNT10"]) echo("<span id='sale'>-10%</span>"); elseif($itemCatalogMeter["UF_DISCOUNT5"]) echo("<span id='sale'>-5%</span>"); elseif($itemCatalogMeter["UF_SALE"]) echo("<span id='sale'>SALE</span>"); else echo("<div style='left: -3px; width:0px;'><span id='lable'>&nbsp;</span></div>");?></div>
        <?if($itemCatalogMeter["UF_NEWCATALOG"]) echo("<div style='position: relative; top: -175px; left:-3px; float:left;'><span id='new'>NEW</span></div>"); ?>
	<?if($itemCatalogMeter["UF_HIT"]) echo("<div style='position: relative; top: -175px; left:-3px;'><span id='hit'>HIT</span></div>"); ?>
	</div>
<?
  }
?>
...
<!-- Конец Список каталогов Метровых обоев -->




<!-- Список каталогов Недорогих обоев -->
<?elseif($filter_cheap):?>
<!--h1 id='page_title' style='margin-left: 10px;'>Недорогие обои<span></span></h1-->
<?
// ФИЛЬТР
$arFilterMain=Array('ACTIVE' => 'Y', 'IBLOCK_ID' => $IBLOCK_ID, 'UF_CHEAP' => true); 
$arFilterDop=Array();
if($section&&$section!=1) $arFilterDop=array_merge($arFilterDop, Array('SECTION_ID'=>$section));
if($filter_price) $arFilterDop=array_merge($arFilterDop, Array('UF_PRICE'=>$filter_price));
if($filter_width) $arFilterDop=array_merge($arFilterDop, Array('UF_WIDTH'=>$filter_width));
...
<?
    ...
    $listCatalogCheap=CIBlockSection::GetList(array('name'=>'asc'), $arFilter, false, $arSelect=array('UF_FABRIKA', 'UF_NEWCATALOG', 'UF_HIT', 'UF_SALE', 'UF_DISCOUNT10', 'UF_DISCOUNT5', 'UF_SALE_OBOI', 'UF_ACTION')); //'left_margin'=>'asc'
    while($itemCatalogCheap=$listCatalogCheap->GetNext()) 
    {
      $numberFilter++;
$saleID="";
$sale="";
$saleID=$itemCatalogCheap['UF_SALE_OBOI'];
if($saleID){
  $res = CUserFieldEnum::GetList(array(), array("ID" => $saleID, "FIELD_NAME" => "UF_SALE_OBOI"));
    if($ar_res = $res->GetNext())
      $sale=$ar_res["VALUE"];
  ...
  ?>
	<div class="catalog-item">
	  <div class="catalog-item-image"><a href="<?=$itemCatalogCheap['SECTION_PAGE_URL']?>"><?if($itemCatalogCheap["PICTURE"]):?><? echo(ShowImage($itemCatalogCheap["PICTURE"], 150, 150, "border='0' title='Открыть каталог'", "", true));?><?else:?><img src="<?=SITE_TEMPLATE_PATH?>/images/default150.gif" width="150px" height="150px" title="Открыть каталог"><?endif?></a></div>
	  <div class="catalog-item-title"><a href="<?=$itemCatalogCheap['SECTION_PAGE_URL']?>"><nobr><b><?=$itemCatalogCheap["NAME"]?></b></nobr></a></div>	  
	  <div  class="catalog-item-title" style="font-size: 13px;">
	    <?
		if($itemCatalogCheap["IBLOCK_SECTION_ID"]) $type=CIBlockSection::GetByID($itemCatalogCheap["IBLOCK_SECTION_ID"]);
		  if($ar_type=$type->GetNext())  echo $ar_type['NAME'];
	    ?>
	  </div>
	  <div style='position: relative; top: -175px; left: -3px; float:left;'><? if($sale) echo("<span id='sale'>SALE ".$sale."</span>");  elseif($itemCatalogCheap["UF_ACTION"]) echo("<span id='sale' style='background-color:#ffce0c;'>ЛУЧШАЯ ЦЕНА</span>"); elseif($itemCatalogCheap["UF_DISCOUNT10"]) echo("<span id='sale'>-10%</span>"); elseif($itemCatalogCheap["UF_DISCOUNT5"]) echo("<span id='sale'>-5%</span>"); elseif($itemCatalogCheap["UF_SALE"]) echo("<span id='sale'>SALE</span>"); else echo("<div style='left: -3px; width:0px;'><span id='lable'>&nbsp;</span></div>");?></div>
        <?if($itemCatalogCheap["UF_NEWCATALOG"]) echo("<div style='position: relative; top: -175px; left:-3px; float:left;'><span id='new'>NEW</span></div>"); ?>
	<?if($itemCatalogCheap["UF_HIT"]) echo("<div style='position: relative; top: -175px; left:-3px;'><span id='hit'>HIT</span></div>"); ?>
	</div>
<?
  }
?>
...
<!-- Конец Список каталогов Недорогих обоев -->
