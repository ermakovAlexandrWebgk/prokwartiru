...
<?
	...
	if($arSite=="/catalog/oboi/" && ($SECTION_ID==3715||$SECTION_TOP==3715)) { $FABRIKI_TYPE_ID=51; $TYPE_ID=5; } /* фрески */
...
//echo($interierList[$i]['NAME']); echo(" ");
	      ...
	      if($arSite=="/catalog/oboi/" && ($SECTION_ID==3715||$SECTION_TOP==3715)) $path="/catalog/oboi/"; /* фрески */
?>
...
<?if($path_pic_detail):?>
      <div class="catalog-interier-card">
       <!-- Заголовок -->
       <? if($path) $title="<a href='".$path."index.php?SECTION_ID=".$interierList[$i]['ID']."'><nobr>".$interierList[$i]['NAME']."</nobr></a>"; else $title="<nobr>".$interierList[$i]['NAME']."</nobr>";?>
	<div class="item-title" style="text-align: center;"><span class='big'><?=$title?></span></div>
	<!-- Конец Заголовок -->
	<?$title_text="<p style='text-align: center;'><span class='big'>".$title."</span></p>";?>
	<?$numberElements++;?>
        <div class="item-image">
