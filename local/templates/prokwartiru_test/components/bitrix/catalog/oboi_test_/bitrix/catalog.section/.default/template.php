...
<?
  ...
  $arSelect=array('UF_FABRIKA','UF_NEWCATALOG', 'UF_HIT', 'UF_DISCOUNT10','UF_SALE','UF_GLUE','UF_MARK');
  ...
  if($type_item = $type->GetNext()){
        if($type_item['UF_DISCOUNT10']) $page_title=$page_title."<span id='sale'>Скидка 10%</span>&nbsp;";
	...
	if($country=GetIBlockSection($fabrika['IBLOCK_SECTION_ID'], 'catalog')) $page_title=$page_title.". <span style='font-size: 80%;'>".$country['NAME']."</span>"; 
        if(($fabrika['NAME']=="Loymina")||($fabrika['NAME']=="Seabrook")) { $delivery="<a href='/about/discounts/'><img src='".SITE_TEMPLATE_PATH."/images/free.png' title='Бесплатная доставка' alt='Бесплатная доставка'  style='width:145px; height:30px; margin-bottom:-5px; margin-left:30px;' /></a>"; $page_title=$page_title.$delivery;}
 }
...
<?if($glue):?>
  <div id="glue"> 
    <p><a href="/catalog/glue/?ELEMENT_ID=<?=$glue['ID']?>"><?if($glue_pic):?><div><img src="<?=$glue_pic?>"></div><?endif?>С этими обоями фабрика рекомендует клей <?if($glue['CODE']=="SEM_MURALE"):?>SEM MURALE<?else:?>KLEO<?endif?></a></p>
  </div>
<?endif?>
<!-- Конец Клей -->
